...
// }
  ...
  fmSort() {
    // sorts the arrary 'in place'. // javascript sort has a weird compare function.
    this.formatMaskArray0.sort(function (a, b) {
      if (a.fmMaskType < b.fmMaskType) {
        return -1;
      }
      if (a.fmMaskType > b.fmMaskType) {
        return 1;
  ...
  fmFilter() {
    ...
    this.formatMaskArray0 =
      this.formatMaskArray0.map(fm => {
        ...
        if (i == -1 || fm.fmMask == 'N/A' || fm.fmMask == '(blank)' || fm.fmMask == '_')  {
...
createMt(){
  ...
  for (i = 0; i < this.formatDetailArray0.length; i++) {
    this.selectedFD = this.formatDetailArray0[i]
    if ( this.selectedFD.fdContentName == 'Literal') {
      fi = this.maskAndTextArray0.findIndex(x => x.mtMaskOrText == this.selectedFD.fdMask)
      if (fi < 0) { // this literal is not yet in the mt array.  insert it.
        this.maskAndTextArray0.push( // we only want unique values pushed into maskAndText.
          {
          id: null, 
          mtMaskOrText: this.selectedFD.fdMask,
          mtContent: this.selectedFD.fdContentName,
          mtMorT: 't'
          }        
        ) // end push
      } // end if fi < 0
     ...
     if ( this.selectedFD.fdContentName != 'Literal') {
      fi = this.formatMaskArray0.findIndex(x => x.fmMask == this.selectedFD.fdMask)
      if (fi == -1) { // this mask is not yet in the formatMaskArray0 array.  insert it.
        if (this.selectedFD.fdMask.toString().indexOf('AMC') > -1) {
          maskType = 'Lot Number'; // has AMC in the mask
        } else {
          maskType = 'Date';  // must be a date
        } //end if AMC wording
        this.formatMaskArray0.push( // we only want unique values pushed into formatMaskArray0.
          {
          id: null, 
          fmMaskType: maskType, 
          fmMask: this.selectedFD.fdMask,
          fmFilterInOut: 'init'
          }        
        ) // end push
      } // end if fi < 0  (check if this mask is in the array)
     } // end if not Literal
  } // end for
}
